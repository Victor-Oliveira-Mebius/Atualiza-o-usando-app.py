...
<body class="bg-gray-50 text-gray-800">
  ...
  <header class="bg-white sticky top-0 z-50 h-20 hidden md:flex shadow">
    <div class="container mx-auto px-4 py-3 flex justify-between items-center h-full">
      ...
      <nav class="hidden md:block">
        <a href="/form.html" class="text-blue-600 hover:underline">Formulário</a>
      </nav>
    </div>
  </header>

  <div class="max-w-6xl mx-auto p-6">
    <h1 class="text-3xl font-bold text-blue-700 mb-6">📬 Mensagens Recebidas</h1>
    ...
    <div id="aviso" class="hidden p-4 mb-4 rounded bg-yellow-50 text-yellow-800"></div>
    <div id="tabela-container"></div>

    <div class="mt-10">
      <a href="/form.html" class="text-blue-600 hover:underline">&larr; Voltar ao formulário</a>
    </div>
  </div>
  ...
  <script>
    ...
    toggleBtn.addEventListener('click', () => {
      ...
      toggleBtn.textContent = filtrosContainer.classList.contains('hidden') ? '🔍 Buscar filtros' : '❌ Ocultar filtros';
    });

    excluirTodosBtn.addEventListener('click', () => {
      alert('As mensagens são armazenadas no banco de dados e não podem ser apagadas por aqui.');
    });
    ...
    function renderTabela(filtro = {}) {
      ...
      clientes.sort((a, b) => new Date(b.data_hora) - new Date(a.data_hora));
      clientes = clientes.filter(c => {
        if (filtro.data && new Date(c.data_hora).toISOString().split('T')[0] !== filtro.data) return false;
        ...
        tabela += `
          <tr class="border-b hover:bg-gray-50 align-top">
            ...
            <td class="py-3 px-4 max-w-xs">
              <div class="relative">
                <p id="${mensagemId}" class="message max-h-24 overflow-hidden transition-all duration-300 whitespace-pre-line">${(c.mensagem ?? '').replaceAll('<','&lt;')}</p>
            ...
            <td class="py-3 px-4 text-sm text-gray-500">${formatarDataDisplay(c.data_hora)}</td>
          </tr>
        `;
      });
    ...
    document.getElementById('filtro-form').addEventListener('input', () => {
    ...
    // ===== BUSCA NA API (PHP) =====
    fetch('/api.php?action=mensagens')
      .then(async res => {
        if (res.status === 403) {
          aviso.classList.remove('hidden');
          aviso.innerHTML = 'Acesso restrito. Abra <a class="underline" href="/acesso.php">/acesso.php</a> e informe o token para visualizar as mensagens.';
          throw new Error('403');
        }
        return res.json();
      })
      ...
      .catch(err => {
        if (String(err.message) !== '403') {
          container.innerHTML = `<div class="text-red-500">Erro ao buscar mensagens no servidor.</div>`;
          console.error(err);
        }
      });

    // Limpa querystring (se houver)
